...
';
 ...
 echo '</fieldset>';
?>
<form action="index2editprezkabinet.php" method="post" enctype="multipart/form-data">
<fieldset>
...
$dbcon=mysqli_select_db($database); 
 ...
 for ($i=0;$i<$numresult;$i++)
 {
   ...
   echo '<tr>';
    echo "<form action=index2editprezkabinet.php  id='".$i;echo"' method=post><input type=hidden name=save value='".$row['id'];echo"'></input>";
 
    echo '<td><input class="input2" name="one" value="'.$row['nomer']; echo'"></td>';

        echo '<td>';
        echo '<input class="input2" type="submit" value="сохранить" ></td></form>';
              echo '<td>';
    echo '<form action=index2editprezkabinet.php> <input type=hidden name=del value ='.$row['id'];echo'></input>';
   ...
   if(isset($_REQUEST['save']))
{
 ...
 $query = "UPDATE kabinets SET nomer = '$one' WHERE id=$id";
 $result = $handle->query($query);


 if($result){echo'<script>alert("Данные успешно изменены");</script><script type="text/javascript">location.replace("index2editprezkabinet.php")</script>';}
}
?>
    ...
    if(isset($_REQUEST['del']))
{
 ...
 $query = "DELETE FROM kabinets WHERE id = $del";
 $result = $handle->query($query);
if($result){echo'<script>alert("Данные успешно удалены");</script><script type="text/javascript">location.replace("index2editprezkabinet.php")</script>';}
}
 ?>
 ...
 $add = $_REQUEST['add'];
    ...
    $group = $_REQUEST['grouppa'];
     ...
     $handle = mysqli_connect($host, $user, $password, $database) ;
 $query="INSERT INTO kabinets (nomer) VALUES ('$group')"; 
 $result = $handle->query($query);
if($result){echo'<script>alert("Данные успешно сохранены");</script><script type="text/javascript">location.replace("index2editprezkabinet.php")</script>';}
}
?>
